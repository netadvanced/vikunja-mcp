...
| E | Finish line: `node-vikunja` removed from package.json; upstream courtesy reports; coverage ratchet; CI re-enable (owner's call); optional Docker packaging | in progress — ✅ **`node-vikunja` dependency removed** (branch `waveD-node-vikunja-removal`): final ~19 runtime call sites migrated to `vikunjaRestRequest`, all type-only imports moved to generated OpenAPI types, client-surface infra deleted, dependency dropped from `package.json`; `grep -rn node-vikunja src/ package.json` is zero. Completes decision 2 below. |
